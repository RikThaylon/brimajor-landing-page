...
interface ContactFormProps {
  ...
  /** Interesse controlado externamente (sobrescreve o estado interno quando fornecido) */
  value?: string;
...
export function ContactForm({ initialInterest = "", value, onChange }: ContactFormProps) {
  ...
  // Modo controlado se `value` for passado
    ...
    <div className="max-w-xl mx-auto mt-12 bg-brimajor-graphite border border-brimajor-techgray p-8 rounded-xl shadow-2xl animate-slide-up delay-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <button
          type="submit"
          disabled={!name.trim()}
          className="w-full mt-2 bg-[#25D366] hover:bg-[#1ebe59] text-white font-medium py-3.5 rounded-md transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_20px_-6px_rgba(37,211,102,0.5)] flex justify-center items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-none"
        >
